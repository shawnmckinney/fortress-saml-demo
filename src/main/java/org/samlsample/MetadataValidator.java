...
public class MetadataValidator implements Validator
{
    ...
    public void validate(Object target, Errors errors)
    {
        ...
        else if ( !"pkix".equalsIgnoreCase( metadata.getSecurityProfile() ) && !"metaiop".equals( metadata
            .getSecurityProfile() ) )
        {
        ...
        else if ( !"pkix".equalsIgnoreCase( metadata.getSslSecurityProfile() ) && !"metaiop".equals( metadata
            .getSslSecurityProfile() ) )
        {
            errors.rejectValue( "sslSecurityProfile", null, "Selected value is not supported." );
        }

        if ( metadata.isIncludeDiscovery() && metadata.getCustomDiscoveryURL() != null && metadata
            .getCustomDiscoveryURL().length() > 0 )
        {
            try
            {
        ...
        if ( metadata.isIncludeDiscovery() && metadata.getCustomDiscoveryResponseURL() != null && metadata
            .getCustomDiscoveryResponseURL().length() > 0 )
        {
            try
            {
